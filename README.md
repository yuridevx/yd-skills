...
| `linked-testplan` | rulebook | (loaded by `duo-testplan` or by explicit reference; no output of its own) | no |
...
- `codex` CLI on PATH. Skills pin model `gpt-5.5`, reasoning `xhigh`, yolo / `--dangerously-bypass-approvals-and-sandbox`. Web search is `live` for most skills. `duo-testplan` is the exception: web search defaults OFF (source code is the only ground truth); flip to `live` with the `web-allowed` prose modifier.
